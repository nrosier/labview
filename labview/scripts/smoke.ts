...
/**
 * Three properties, each of which this file went without for too long:
 *
 * **An assertion is isolated.** `cond` is a thunk rather than a boolean, so a throw while
 * computing it — `lookup()` no longer finding a service, a fixture that stopped parsing,
 * a `svc.probe` gone `undefined` — is *this* assertion failing rather than the process
 * ending. Running 1,300 assertions is only worth anything if a break in the first one
 * still leaves the other 1,299 reported; before this, one bad fixture read hid the state
 * of everything below it. The thunk is mandatory rather than `boolean | (() => boolean)`
 * on purpose: `tsconfig.scripts.json` typechecks this file, so the signature is what
 * stops a future call site from quietly opting back out.
 *
 * **An assertion is counted.** A summary that reports only failures cannot tell 1,361
 * assertions from three, so an early return, a skipped block, or a section that stopped
 * running reads exactly like success — which is §8's own worry (an assertion on
 * `undefined` can pass while proving nothing) arriving through the back door. The guard
 * is a number, and it is cheap.
 *
 * **A section is structure, not a print statement.** `section()` closes the previous one,
 * which is what makes the last guard possible: a section that ends having asserted
 * nothing is itself a failure.
 */
let passes = 0;
let failures = 0;
let sectionsClosed = 0;
let phases = 0;
const failedNames: Array<{ where: string; name: string }> = [];
...
/**
 * What a failing assertion prints after its name: a literal, or a thunk that reads the
 * value which just disagreed. The thunk may return `undefined` because the thing being
 * read is so often optional (`svc.image`, `warnings[0]`) — printing nothing is the right
 * answer there, and the alternative is `?? ""` at a hundred call sites. This is also the
 * one behaviour the old eager `detail = ""` parameter had that was worth keeping.
 */
type Detail = string | (() => string | undefined);
type AsyncDetail = string | (() => string | undefined | Promise<string | undefined>);

/** What a `detail` that threw says instead, so a diagnostic cannot become the failure. */
function detailThrew(err: unknown): string {
  return `<detail threw: ${err instanceof Error ? err.message : String(err)}>`;
}

function resolveDetail(detail: Detail | undefined): string {
  if (detail === undefined) return "";
  if (typeof detail === "string") return detail;
  // The detail almost always reads the value the condition just read, so it is as
  // capable of throwing as the condition is — and a lazy condition with an eager
  // detail would still take the process down.
  try {
    return detail() ?? "";
  } catch (err) {
    return detailThrew(err);
  }
}

/** The same, for the six details that must await. Separate for `check`'s reason below. */
async function resolveDetailAsync(detail: AsyncDetail | undefined): Promise<string> {
  if (detail === undefined) return "";
  if (typeof detail === "string") return detail;
  try {
    return (await detail()) ?? "";
...
function fail(name: string, detail: string): void {
  failures++;
  currentSectionCount++;
  failedNames.push({ where: currentSection ?? currentPhase, name });
  console.error(`  ✗ ${name} ${detail}`);
}

function threw(name: string, err: unknown): void {
  failures++;
  currentSectionCount++;
  failedNames.push({ where: currentSection ?? currentPhase, name });
...
/** One named assertion. See the harness note above for why `cond` is a thunk. */
function check(name: string, cond: () => boolean, detail?: Detail): void {
  let ok: boolean;
  try {
    ok = cond();
  } catch (err) {
  ...
  if (ok) pass(name);
  else fail(name, resolveDetail(detail));
}

/**
 * The same, for the conditions that must await. Split rather than making `check` async
 * because 1,109 of the 1,139 sites are synchronous and `await check(...)` at every one of
 * them would be a footgun the day someone forgets it: a floating promise is truthy, so a
 * forgotten `await` is an assertion that passes no matter what it was asked.
 */
async function checkAsync(
  ...
  if (ok) pass(name);
  else fail(name, await resolveDetailAsync(detail));
}

/** Close the open section, failing it if it proved nothing. */
function closeSection(): void {
  if (currentSection === null) return;
  sectionsClosed++;
  if (currentSectionCount === 0) {
    failures++;
    failedNames.push({ where: currentPhase, name: `section "${currentSection}" asserted nothing` });
    console.error(`  ✗ section "${currentSection}" ran to its end without asserting anything`);
  }
  currentSection = null;
}

/** A group of assertions under one heading. */
...
/**
 * One fixture root's worth of sections. `root` is required because every phase has one,
 * and naming it in the banner is what lets a failure be traced to the fixtures that
 * produced it — the seven roots are the suite's actual top-level structure.
 */
function phase(title: string, root: string): void {
  closeSection();
  phases++;
  currentPhase = `${title} (${root})`;
  console.log(`\n--- ${title} (${root}) ---`);
}
...
function report(): never {
  ...
  console.log(
    `\n${failures === 0 ? "PASS" : "FAIL"} — ${ran} assertion(s) in ${sectionsClosed} section(s)` +
      ` across ${phases} phase(s), ${failures} failure(s)`,
  );
  if (failures > 0) {
    console.error("\nfailed:");
...
// No trailing blank line: section() and phase() open with one of their own.
...
phase("representative fleet", "fixtures/apps");
...
section("ingress classification");
...
section("secret masking + interpolation");
...
section("tunnel origin resolution");
...
section("graph draws the path, not a shortcut");
...
section("published host ports are LAN reachability");
...
section("an unprovable tunnel origin stays unproven");
...
section("provider attribution needs proof, not a name");
...
section("operator declarations (.labview)");
...
section("sidecar validation, asserted directly");
...
section("the sidecar's reading of a service reference");
...
phase("network connections", "fixtures/nets");
...
section("one external network across four stacks");
...
section("the dependency compose cannot express");
check(
...
section("sharing a network is not a dependency");
check(
...
section("a dependency drawn through the network that carries it");
...
section("a dependency with no network to travel over");
...
section("what a reference can turn out to name");
check(
...
section("networks that connect nothing");
...
section("the caps on a network too large to draw");
...
phase("identity provider API", "fixtures/authentik");
...
section("the token goes nowhere it has not been earned");
...
section("the applications endpoint withholds part of its own list");
check(
...
section("match 1: the provider names the service (internal host)");
...
section("match 2: an address inside a URL the provider hands out");
...
section("match 3: a hostname both sides name");
...
section("match 4: a name, when it points at exactly one service");
...
section("a name too generic to identify anything matches nothing");
...
section("an ambiguous name is discarded, not arbitrated");
check(
...
section("a shared authentication domain identifies no single service");
...
section("a provider no outpost serves enforces nothing");
...
section("a confirmed gate with no method to report it as");
...
section("endpoint from configuration");
...
section("without a token the integration is inert");
...
section("a token variable that arrived empty is said out loud");
...
// provider buys.
section("what the provider's records are worth");
check(
...
section("a superuser token is served the list itself");
...
section("a gap recovery closes completely is not reported as a gap");
...
section("an unreachable provider never blocks a scan");
...
phase("reverse proxy API", "fixtures/traefik");
...
section("a file-provider middleware the scan cannot see");
...
section("the downgrade, and the guard on it");
...
section("routers the labels and the proxy disagree about the existence of");
...
section("three accounts of one gate");
...
section("a credential goes nowhere ownership was not established");
...
section("a gated API, reached with the credential it demands");
...
section("half a credential is reported, never completed by guesswork");
...
section("a partial read concludes nothing about a missing gate");
...
section("an unreachable proxy never blocks a scan");
...
section("without the API the integration is inert");
...
section("what the proxy's runtime configuration is worth");
check(
...
phase("active probe", "fixtures/probe");
...
section("which addresses may be asked at all");
...
section("...and in which order, since the address decides what an answer is worth");
...
section("what counts as a login page — and what does not");
/*
...
section("the second request, and the one thing it is allowed to decide");
/*
...
section("what a login form is made of");
/*
...
section("what the page showed a caller who sent nothing");
/*
...
section("where a response was pointing, read once for both the verdict and the record");
/*
...
section("why an answer was read the way it was");
/*
 ...
 */
section("the report on a rule says what the rule said");
/**
 ...
 * sent for nothing.
 */
section("and what it finds where the rule does not look");
...
section("what the lab can say about a page the rule found nothing on");
/*
 ...
 */
section("what counts as authentication this scan detected, before anything is asked");
...
section("off by default, and off means nothing left the process");
probeEnv({});
...
section("a login page answering is protection LabView measured");
...
section("and an answer with no login page is the other half of the value");
check(
...
section("the one page no reading of a body can judge, and the address behind it");
/*
...
section("the page a stranger was shown, and the one thing it can prove");
/*
...
section("a service whose authentication this scan already found is not asked at all");
/*
...
section("nothing was measured about a service that did not answer");
...
section("the first address that answers wins, and the reader is told which");
...
section("the panel lists exactly the services the tile counted");
/*
...
section("whether the probe ran is part of the payload, not something a reader infers");
/*
...
section("a measurement against a declaration that was holding a finding back");
...
section("what the probe never sent, which is the half that cannot be inferred");
...
section("the exposed count can still be reconstructed without the probe (I1)");
/*
...
section("a transport failure names the stage, not just `fetch failed`");
...
section("an error status names the stage too");
...
section("the shared HTTP client carries the phase to its caller");
...
section("the docker socket file is diagnosed before dockerode sees it");
...
section("dockerode's own failures map to the same phases");
...
section("both docker transports are named the way an operator writes them");
...
await checkAsync(
  ...
  async () => dockerCfg({}).docker.socketPath === "/var/run/docker.sock" &&
...
await checkAsync(
  ...
  async () => (await snapshotDocker(dockerBase)).connection.phase === "disabled" &&
    !shouldBanner((await snapshotDocker(dockerBase)).connection),
);
socketServer.close();

section("a container the Engine would not describe is a gap, and says so");
...
section("every target reports, in the order LabView reads them");
traefikEnv({});
authentikEnv({ token: AK_TOKEN });
...
section("the phase reported is the furthest any candidate got");
...
section("an integration nobody switched on says so quietly");
traefikEnv({ enabled: false });
authentikEnv({});
...
section("repeat scans do not repeat themselves");
...
section("no diagnostic carries a credential");
...
section("a forced rescan is never answered by a scan that started before it");
...
await checkAsync("...and both are answered by the one scan", async () => (await firstCaller) === "scan-1" && (await alongside) === "scan-1");
check(
...
await checkAsync("inside the TTL nothing is re-read", async () => (await h.cache.get(false)) === "scan-1" && h.started === 1);
h.advance(6_000);
...
await checkAsync(
  ...
  async () => (await afterTtl) === "scan-2" && h.built[1]?.prev === "scan-1" && h.built[1]?.forced === false,
  async () => JSON.stringify(h.built[1]),
);
...
await checkAsync("the in-flight scan answers the passive caller", async () => (await running) === "pre-edit-scan");
check("...and the click gets a scan of its own, started after it asked", () => h.started === 4);
await h.finish("post-edit-scan");
await checkAsync("...whose result is what the click returns", async () => (await clicked) === "post-edit-scan");
...
await checkAsync("...and both are answered by it", async () => (await clickA) === "scan-5" && (await clickB) === "scan-5");
...
await checkAsync("...successfully", async () => (await retried) === "scan-7");

section("a per-rescan override travels with the caller that started the build");
/*
...
await checkAsync("both are answered by the build that ran", async () => (await askedOn) === "probed" && (await tagAlong) === "probed");
overrideClock += 6_000;
...
await checkAsync("...and that is what the next reader sees", async () => (await timerRebuild) === "configured");

section("what a rescan found, compared over the parsed configuration");
...
section("the same, through the whole pipeline, with an Engine that answers differently each time");
...
section("a rescan of a root that really changed on disk");
...
section("the rescan line says what moved, and says so when nothing did");
check(
...
section("a rescan also says what the integration reads came back with");
...
section("the same holds for the proxy, in its own vocabulary");
traefikEnv({});
authentikEnv({ token: AK_TOKEN });
...
section("when a rescan speaks, and when it stays quiet");
...
section("no rescan line carries a value out of the configuration");
...
section("the ingress vocabulary normalizes on the way in");
...
section("the tag filter: OR, AND and NOT");
...
section("the view as a query string: what a link may say");
...
section("the ingress palette and the stylesheet name the same variables");
...
section("which build is running, and how confidently that is known");
...
section("a missing gate is only reported where a gate was expected");
...
// ---------------------------------------------------------------------------------
phase("access control", "fixtures/auth");
...
section("the algorithm is whatever the hash says it is");
{
  ...
  await checkAsync("each of the three prefixes verifies its own password", async () => (await Promise.all([
    verifyPassword("alice-secret", alice),
    verifyPassword("bob-secret", bob),
    verifyPassword("carol-secret", carol),
  ])).every(Boolean));
  await checkAsync("a wrong password does not", async () => (await verifyPassword("alice-secretx", alice)) === false);
  await checkAsync(
    "an unusable hash is a wrong password, not an error — a caller that could tell them apart could probe the file",
    async () => (await verifyPassword("hunter2", "hunter2")) === false && (await verifyPassword("x", alice.slice(0, 40))) === false,
  );

  const minted = await hashPassword("round-trip", 4);
  await checkAsync(
    "a freshly hashed password round-trips, at $2b$ and the cost it was asked for",
    async () => hashAlgorithmId(minted) === "2b" && hashCost(minted) === 4 && (await verifyPassword("round-trip", minted)),
  );
  check("the default cost is what `hashpw` will use, not what the fixtures use", () => DEFAULT_COST === 12);
  await checkAsync("a cost below bcrypt's own floor is clamped rather than rejected", async () => hashCost(await hashPassword("x", 1)) === 4);
  check(
  ...
  await checkAsync("...and is minted once per cost, not per attempt", async () => (await decoyHash(4)) === decoy);
  await checkAsync("...and differs per cost, because that is the point of it", async () => (await decoyHash(5)) !== decoy);
}

section("a passwd file names its own mistakes");
{
  ...
  await checkAsync("the first entry is the one that verifies", async () => await verifyLogin(messyFile, "frank", "other-secret"));
  await checkAsync("...and the shadowed duplicate's password is not accepted", async () => (await verifyLogin(messyFile, "frank", "long-secret")) === false);
  ...
  const okFile = readPasswd(passwdOk);
  await checkAsync("an unknown username is refused", async () => (await verifyLogin(okFile, "nobody", "alice-secret")) === false);
  await checkAsync("...as is a username the format would not allow at all", async () => (await verifyLogin(okFile, "al ice", "alice-secret")) === false);
  await checkAsync("...and an absurd password is refused without being hashed", async () => (await verifyLogin(okFile, "alice", "x".repeat(2000))) === false);
  await checkAsync("a known username with its own password is accepted", async () => await verifyLogin(okFile, "alice", "alice-secret"));
  await checkAsync("...with surrounding whitespace forgiven on the name, never on the password", async () => await verifyLogin(okFile, "  alice  ", "alice-secret"));
...
section("reading the file, including the ways it goes wrong");
{
...
section("a session token is signed, dated and revocable");
{
...
section("the cookie that carries it");
{
...
section("failed sign-ins are counted per username, not per address");
{
...
section("open unless configured");
{
...
section("the posture comes from the file, read as configured");
{
...
section("an override is a copy of the configuration, never an edit to it");
{
  /*
...
section("settings LabView no longer reads");
{
...
section("what may be read without a session");
{
...
section("single sign-on: the parts that are pure arithmetic");
{
...
section("single sign-on: the provider's documents");
{
...
section("single sign-on: an ID token is not believed until it verifies");
{
  const ISSUER = "https://idp.example.com/application/o/labview/";
...
section("single sign-on: the token exchange, against a stubbed provider");
{
  const ISSUER = "https://idp.example.com/application/o/labview/";
  const CLIENT = "labview";
  const NONCE = "the-nonce-of-this-attempt";
...
section("the gate itself, driven through real requests");
{
  ...
  await checkAsync("a rescan with no body at all behaves as it always did: configuration decides", async () => (await probeOf()) === "200 false config", async () => await probeOf());
  await checkAsync(
    "a body asking for the probe turns it on for that build, and says a request decided",
    async () => (await probeOf({ probe: true })) === "200 true request",
    async () => await probeOf({ probe: true }),
  );
  await checkAsync(
    ...
    async () => (await probeOf({ probe: false })) === "200 false request",
    async () => await probeOf({ probe: false }),
  );
  await checkAsync(
    "a probe that is not a boolean is not an answer, so configuration decides again",
    async () => (await probeOf({ probe: "yes" })) === "200 false config",
    async () => await probeOf({ probe: "yes" }),
  );
  await checkAsync(
    "...and neither is an array, an unknown key, or a JSON null where an object goes",
    async () => (await probeOf([true])) === "200 false config" &&
      (await probeOf({ probing: true })) === "200 false config" &&
      (await probeOf("null")) === "200 false config",
    async () => `${await probeOf([true])} / ${await probeOf({ probing: true })} / ${await probeOf("null")}`,
  );
