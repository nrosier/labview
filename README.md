...
- Auth detection is label/env-driven, and heuristic wherever neither API is
  ...
  Grafana or Gitea also does — still reads as exposed. That direction is deliberate: a login page LabView cannot
  ...
  because that is where the misses turn out to be. Two of its findings have since become
  rules: the flow-executor paths a redirect lands on, and the eighth signal itself.
